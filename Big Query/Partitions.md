
*Table partitioning*

Partitioning is when you separate data in storage

It can optimized how tables are stored and queried

Splits the table into smaller pieces called partitions, based on certain criteria

Can partition by things like timestamps, ingestion time, integer ranges 

Reduces the amount of data that is scanned, improving performance and lowering query costs

Good for large and fast-growing data sets

*Common time-partition sources*

1) IoT sensor data
2) Transaction logs
3) Clickstream data (user interactions with sites and apps)
4) Sensor logs

Usually partitioned by hour or day

Can set partition expirations to be deleted once 